...
int dmg;
{
    ...
    case AD_LOUD:
        if (m_canseeu(mtmp)) {
            pline("%s lets out an ear-splitting screech!", Monnam(mtmp));
        } else {
            You_hear("a painfully loud screech!");
        }
        if (u.usleep)
            unmul("You wake up.");
        stop_occupation();
        aggravate();
        losehp(d(2, 4), Hallucination ? "good vibrations" : "sonic vibrations", KILLED_BY);
        break;
    default:
        break;
    }
